...
void registerBlockInBlockCache(uint8_t* block, uint16_t block_number) {
  // TODO: There is an inefficiency in our Block Cache implementation that stems from the fact that historically we placed a few fields
  //   that are core to Block DB inside the FlatBuf structure.
  // Namely, those are `BlockVersion` and `Tombstoned` flag.
  // Since those values are not accessible directly from the block header, like BlockType is for example, we can't run 
  //   `2. Update DB structures` before we decode FlatBuf, because we need to know Version and Tombstoned status for that.
